...
MEMORY_REVIEW_PROMPT = (
    "You are reviewing ClawShell edge brain memories. These are summaries of what edge agents "
    "(Hermes, Wukong) have learned, decided, or discovered during their sessions. "
    "Identify recurring themes, knowledge gaps, optimization opportunities, and cross-edge patterns. "
...
class CloudAnalyst:
    ...
    def _get_memory_events(self, since: float, limit: int = 100) -> List[dict]:
        """Extract memory.stored events from EventBus for edge memory review."""
        if not self._eventbus:
            return []
        try:
            return self._eventbus.query(
                event_type="memory.stored", since=since, limit=limit
            )
        except Exception:
            return []

    def review_edge_memories(self, since_hours: float = 6) -> dict:
        """复盘端脑记忆：分析Edge产出的记忆事件，生成知识洞察。"""
        since = time.time() - (since_hours * 3600)
        mem_events = self._get_memory_events(since=since, limit=100)

        if not mem_events:
            return {"success": True, "content": "No edge memories in review window.", "memories": 0}
        ...
        for e in mem_events:
            ...
            summaries.append({
                ...
                "preview": p.get("content_preview", "")[:150],
                "node": p.get("node_id", "unknown"),
                "time": e.get("timestamp", 0),
            })

        ctx = json.dumps(summaries, indent=2, default=str, ensure_ascii=False)
        result = self._llm.chat(
            MEMORY_REVIEW_PROMPT,
            f"Review {len(summaries)} edge brain memories from the last {since_hours}h:\n\n{ctx}\n\n"
            "Analyze: 1) Recurring themes 2) Knowledge gaps 3) Optimization opportunities 4) Cross-edge patterns.",
            temperature=0.5,
        )
    ...
    def deep_review(self):
        ...
        if self._eventbus and hasattr(self._eventbus, 'get_stats'):
            try:
                ...
                pass
        # v1.12.0: 包含端脑记忆事件
        mem_events = self._get_memory_events(since=now - self.DEEP_REVIEW_INTERVAL, limit=50)
        mem_ctx = ""
        if mem_events:
            mem_summaries = [{"category": e.get("payload",{}).get("category",""),"preview": e.get("payload",{}).get("content_preview","")[:100]} for e in mem_events]
            mem_ctx = f"\nEdge Memories ({len(mem_events)}):\n{json.dumps(mem_summaries, indent=2, ensure_ascii=False)}"
        return self._llm.chat(SYSTEM_PLANNER, f"Deep Review - Last 6 Hours\nSystem Stats:\n{stats}\nRecent Events ({len(events)}):{mem_ctx}\nAnalyze holistically. Provide architecture recommendations.", temperature=0.5, max_tokens=8192)
