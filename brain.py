...
@brain_router.post("/review/memories")
async def review_edge_memories(request: Request):
    """复盘端脑记忆 — 分析 Edge 产出的 memory.stored 事件。"""
    body = await request.json()
    hours = body.get("hours", 6)
    result = _get_brain(request).review_edge_memories(since_hours=float(hours))
    return format_api_response(result.get("success", False), data=result)
