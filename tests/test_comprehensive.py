...
def test_all():
    ...
    if _fastapi_ok:
        ...
        check("CloudConfig.safe masks secrets",
              "****" in json.dumps(cloud_config.to_dict(safe=True)) or not any(
                  cloud_config.to_dict(safe=False).get(k) for k in cloud_config.to_dict(safe=False)
    ...
    eb.subscribe("error.*", _event_handler)
    ...
    assert FAILED == 0, f"{FAILED} test(s) failed out of {TOTAL}"