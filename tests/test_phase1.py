...
def test_all():
    ...
    try:
        ...
        # Track handler calls
        handler_calls = []
        def _cron_handler(task):
            handler_calls.append(task["task_id"])

        scheduler.set_handler("_cron_handler", _cron_handler)

        # Register a task
        tid = scheduler.register_task(
            "test-task", "* * * * *", "Test task", "_cron_handler"
        )
    ...
    safe_d = config.to_dict(safe=True)
    raw_d = config.to_dict(safe=False)
    has_secrets = any(raw_d.get(k) for k in raw_d if any(s in k.lower() for s in ("secret", "token", "key", "password")))
    check("Config safe dict masks secrets when present", "****" in json.dumps(safe_d) or not has_secrets)
    ...
    assert failed == 0, f"{failed} test(s) failed out of {passed + failed}"